        ...
        <aside class="sidebar">
            ...
            <nav>
                ...
                <a href="transfer.php" class="btn dash-text">
                    <img src="../assets/images/hover-transfer.png" alt="transfer-logo" class="nav-icon"
                        ...
                        <div class="sum-warp">
                            ...
                            <form method="GET" action="">
                                <select name="type" id="type" onchange="this.form.submit()">
                                    <option value="transfer_in" <?= $type === 'transfer_in' ? 'selected' : '' ?>>
                                        Transfer In</option>
                                    <option value="transfer_out" <?= $type === 'transfer_out' ? 'selected' : '' ?>>
                                        Transfer Out</option>
                                </select>
                            </form>
                        </div>
            ...
            <div class="transactions-table-wrapperm content-1">
                <form method="get" id="filter-form">
                    <select name="type" id="type-filter" onchange="document.getElementById('filter-form').submit()">
                        ...
                        <option value="transfer_in" <?= ($_GET['type'] ?? '') === 'transfer_in' ? 'selected' : '' ?>>
                            Transfer In</option>
                        <option value="transfer_out" <?= ($_GET['type'] ?? '') === 'transfer_out' ? 'selected' : '' ?>>
                            Transfer Out</option>
                    </select>
                </form>
                ...
                <table class="transactions-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td
                                class="amount <?= in_array($txn['type'],['deposit','transfer_in'])? 'positive':'negative' ?>">
                                <?= (in_array($txn['type'],['deposit','transfer_in'])? '+':'−') .
                                          '₹'.number_format($txn['amount'],2) ?>
                            </td>
                            <td>
                                <button
                                    onclick="window.location.href='generate_receipt.php?transaction_id=<?= htmlspecialchars($txn['transaction_id']) ?>'"
                ...
                <div class="pagination">
                    ...
                    <a href="?page=<?= $currentPage - 1 ?><?= $typeFilter ? '&type=' . urlencode($typeFilter) : '' ?>"
...
document.addEventListener("DOMContentLoaded", () => {
    fetch('get_monthly_transfer_in.php')
        ...
        .then(data => {
            ...
            const depositMap = data.reduce((acc, {
                month,
                total_deposit
            }) => {
